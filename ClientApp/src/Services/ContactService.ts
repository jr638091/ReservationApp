import { HttpClient } from "@angular/common/http";
import { Inject, Injectable } from "@angular/core";
import { Observable } from "rxjs";

@Injectable({ providedIn: "root" })
export class ContactService {
  httpClient: HttpClient;
  url: string;

  constructor(http: HttpClient, @Inject("BASE_URL") base_url: string) {
    this.httpClient = http;
    this.url = base_url + "contact";
  }

  list(options: Object = {}): Observable<Pagination<Contact>> {
    return this.httpClient.get<Pagination<Contact>>(this.url, options)
  }

  partialUpdate(id: Number, patch: Object, options: Object = {}) {
    return this.httpClient.patch(this.url + `/${id}`, patch, options)
  }

  create(contact: Object): Observable<Contact> {
    return this.httpClient.post<Contact>(this.url, contact)
  }

  delete(id: Number): Observable<Object> {
    return this.httpClient.delete(`${this.url}/+${id}`)
  }
}
